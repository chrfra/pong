		...
		MouseWheelListener
{
	...
	public Trackball(Player player, Camera camera)
	{
		trackballSize = 0.8f;
		this.player = player;
		this.camera = camera;
	...
	public float[] getRotMatrix()
	{
		...
		Quaternion curQuat2 = new Quaternion(curQuat[1], curQuat[2],
				curQuat[3], curQuat[0]);
		//player.rotateByQuaternion(curQuat2);
		camera.rotateByQuaternion(curQuat2);
		if (spin)
		{
	...
	{
		Quaternion q = new Quaternion(curQuat[1], curQuat[2], curQuat[3],
				curQuat[0]);
		return q;
	}
	...
	/**
	 * Given two rotations, e1 and e2, expressed as quaternion rotations, figure
